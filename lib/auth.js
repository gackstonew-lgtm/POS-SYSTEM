...
exports.handler = async (event) => {
  ...
  if (typeof event.body === 'object' && event.body !== null) {
    ...
    try {
      ...
      return jsonResponse(400, { success: false, message: 'Invalid JSON payload in request body.' });
    }
  }
  ...
  try {
    if (method === 'POST' && (rawPath.endsWith('/login') || rawPath.endsWith('/auth') || !rawPath.includes('/'))) {
    ...
    if (method === 'GET' && (rawPath.endsWith('/me') || rawPath.endsWith('/auth'))) {
    ...
    if (method === 'POST' && rawPath.endsWith('/logout')) {
